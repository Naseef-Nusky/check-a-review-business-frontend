  ...
  return (
    <section ref={sectionRef} className="trust-stats-section border-y border-white/10">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <h2
          className={`trust-stats-title mx-auto max-w-3xl text-center text-2xl font-semibold tracking-tight text-white sm:text-3xl ${
            inView ? 'is-visible' : ''
          }`}
        >
          {title}
        ...
        <div className="mt-12 grid gap-10 sm:grid-cols-3 sm:gap-0">
          {stats.map((stat, index) => (
            <div
              key={stat.label}
              className={`trust-stat-item text-center sm:px-8 ${
            ...
            >
              ...
              {stat.detail ? <p className="mt-1 text-sm text-slate-300">{stat.detail}</p> : null}
            </div>
          ))}
        </div>
      </div>
